...
fun buildGrades(grades: Map<String, Int>): Map<Int, List<String>> {
    val list_of_grades = mutableMapOf<Int, List<String>>()
    for ((key, value) in grades) {
        if (list_of_grades[value] == null) list_of_grades[value] = listOf(key)
        else list_of_grades[value] = list_of_grades[value]!! + key
    }
...
fun mergePhoneBooks(mapA: Map<String, String>, mapB: Map<String, String>): Map<String, String> {
    val mapAB = mapA.toMutableMap()
    for ((key, value) in mapB) {
        if (key in mapA) {
            if (value == mapA[key]) continue
            else mapAB[key] += ", ${value}"
        } else mapAB[key] = value
    }
    return mapAB
}
//if (key in mapA && value != mapA[key]) mapAB[key] += ", ${value}"
//        else mapAB[key] = value
/**
 * Средняя (4 балла)
 *
